...
function generator:SpawnCelestial(data,parent,seed)
	MsgN("CELESTIAL: ",data.name)
	local body = false
	if data.type=='star' then
		local class = string.lower( data.class or 'u')
		local classinfo = self.starclasses[class]
		MsgN("STAR:"..class)
		PrintTable(classinfo)
		local star = ents.Create("star") 
		star.szdiff = 20000
		star:SetSeed(data.seed or (seed + 3218979))
		star:SetName( data.name or parent:GetName())
		local r = data.radius
		local c = JVector(classinfo.color)/255
		star:SetParameter(VARTYPE_RADIUS,r)
		star:SetParameter(VARTYPE_COLOR,c)
		star.radius = r
		star.color = c
	 	if data.mass then star:SetMass(data.mass) end 
	...
	elseif data.type=='planet' then
		...
		body:SetParameter(VARTYPE_ARCHETYPE,data.archetype or "gen_barren")
		if data.archdata then
			body:SetParameter(VARTYPE_ARCHDATA,json.ToJson(data.archdata))
		end
	elseif data.type == 'masscenter' then

		local mass = data.mass
		local center = ents.Create("mass_center")  
		center:SetSeed(seed + 3218888)   
		center:SetParent(parent) 
		center:SetParameter(VARTYPE_MASS,mass)
		center.mass = mass 
		center.radius = 1000
		body = center
	end
	...
	if body then 
		if data.rings then
			body.ringsdata = data.rings
		end
		if data.rotation then 
			body:SetAng(JVector(data.rotation))
		end
		if data.orbit  then--and body.orbit
			if not body.orbit then  
				body.orbit =  body:AddComponent(CTYPE_ORBIT)    
			end
			local o = data.orbit 
			if o.b then
				body.orbit:SetOrbit(
			...
			body.orbit:SetParameter("ms",1*body.orbit:GetOrbitalSpeed(parent.mass,body.mass))
			local psz = body.orbit:GetHillSphereRadius(parent.mass,body.mass)
			body:SetSizepower(psz)

			body.szdiff = psz/body.radius
			body.orbitIsSet = true
		end

		body:Spawn()
		body:SetUpdating(true,16) 

		if data.subs then
			for k,v in pairs(data.subs) do
				self:SpawnCelestial(v,body,seed)
			end 
		end
	end
end
