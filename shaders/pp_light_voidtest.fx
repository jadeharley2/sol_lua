...
{   
   ...
   ; //*((invpos.x*1000%1>0.9)+(invpos.y*1000%1>0.9)+(invpos.z*1000%1>0.9)); 
    ...
    float radli = ldt<5;
    float lvdiz = pow(1-saturate(dot(V,N)),2);
    light_power = saturate(5-ldt)
   // +radli*((apos.x*1000%1>0.95)+(apos.y*1000%1>0.95)+(apos.z*1000%1>0.95))*2
   ;
    float lvdiz2 = 1-saturate(dot(V,N));
    float3 refrRay = refract(V,N,0.9);
    float3 reflRay = reflect(V,N);
    float3 rggb = lerp(
            tEnvView.Sample(sView, refrRay),
            tEnvView.Sample(sView, reflRay),
            lvdiz2)
            +lvdiz*lightColor/10
           ;


    output =  (light_power*rggb)*5;//*surfcolor/100;
	return float4(output,light_power);//*mask.x*0.1;
